...
<!DOCTYPE html>
    ...
    <div class="outer">
        <div class="header" align="center">
            <p style="font-size: 35px; font-weight:bold;">상품문의</p>
            <br><br>
        ...
        <div id="right-side" align="center">
	        <table class="qna-area" align="center">
	            <thead  align="center">
	            ...
	            <tbody align="center" class="qna-list">  
		     		<div class="qna-question" style=" font-size: 10px;" id="answer-info">
	            	<% if(!qnalist.isEmpty()&&loginMem.getMemId()!=null){%>
	            	<!-- 리스트가 비어있지 않을 경우 직접작성한 회원은 작성한 글, 관리자는 전체조회 가능-->
		     			<% for(Board b : qnalist){ %>
		     				<% if(loginMem.getMemId().equals(b.getMemId())||loginMem.getMemId().equals("admin")){ %>
								<tr>
									...
									<td>상품문의</td>
				                    <td>
                    					<div class="qna-div"><%=b.getBoardTitle() %>
			     				...
			     				<tr>
			     					...
			     					<td>질문답변</td>
				                    <td>
				                    	<div class="qna-div">> re : <%=b.getBoardTitle() %>에대해 답변 작성되었습니다.
											<input type="hidden" id="qna-bno" value="<%=b.getBoardNo() %>">					                    	
				                    	</div>				                    	
				                    </td>
				                    <td>관리자</td>
         			...
         			<%} else { %>
		            	...
		            	<tr>
		                    <td colspan="5">
		                        <p>관심있는 상품에 대해 문의해보세요<br><a href="<%=contextPath %>/list.md?currentPage=1">다양한 상품 구경하러 가기</a>                     
		                        </p>
		                    </td>
		                </tr>
				  	<%} %>
          ...
          <div align="center" class="paging-area">
        	...
        	<button onclick="location.href='<%=contextPath %>/list.qna?currentPage=<%=pi.getCurrentPage()-1 %>'">&lt;</button>
        	<%} else { %>
        	...
        	<%for(int i=pi.getStartPage();i<=pi.getEndPage(); i++){ %>
        		...
        		<%} else { %>
        		<button onclick="location.href='<%=contextPath %>/list.qna?currentPage=<%=i %>'"><%=i %></button>
        		<%} %>
        	<%} %>
	...
	<script>
    		$(function(){
    			$(".qna-div").click(function(){
    				...
    				location.href="<%=contextPath %>/detail.qna?bno="+bno;
