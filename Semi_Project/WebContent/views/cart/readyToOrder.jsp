<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="com.semi.cart.order.model.vo.Order, 
    com.semi.cart.model.vo.Cart, java.util.ArrayList, com.semi.cart.controller.OrderPaymentCtlr"%>
    
<% 
	ArrayList<Cart> buyList = (ArrayList<Cart>)request.getAttribute("buyList");
...
<%@ include file="../common/menubar.jsp" %>
    <div class="outer">
        ...
        <div id="item-list">
            ...
            <div style="display:none;">
            	<input type="checkbox" name="cartNo" class="cartNo" value="<%=buyList.get(i).getCartNo() %>" checked>
            </div>
            <%} %>
                    <div class="rt-odrpay-li">
                        ...
                        <table class="odrpay-tbl">
                            ...
                            <tbody align="center"  style="background-color: white;">
                                <%for(Cart c : buyList) {%>
                                <tr style="font-size: 13px; height: 80px;">
                                    <td><%=c.getProductNm() %></td>
                                    <td><%=c.getStrNm() %></td>
                                    <td><%=c.getAmount() %></td>
                                    <td><%=c.getTotalPrice() %></td>
                                    <% totalPrice += (c.getAmount()*c.getTotalPrice()); %>
                                    <% if(c.getEventDate()!=null){%>
                                    <td><%=c.getEventDate() %></td>
                                    <% } else { %>
                                  
                                  		<% storeTotal += (c.getAmount()*c.getTotalPrice());  %>
                                  		<%if(storeTotal>50000){ %>
                                		<td>무료</td>
                               			<% } else { %>
                               			<%	count++; %>
                    ...
                    <div class="delivery">
                        ...
                        <table class="delinfo">
                            <tr>
                                <th>주문자 이름</th>
                                <td><input type="text" name="memName" id="memName" value="<%=loginMem.getMemName() %>" required></td>
                            </tr>
                            <tr>
                                <th>주문자 전화번호</th>
                                <td><input type="text" name="memPhone" id="memPhone" value="<%=loginMem.getMemPhone() %>" required></td>
                               
                            </tr>
                            <tr align="center">
                                <th>주문자 이메일주소</th>
                                <td><input type="text" name="memEmail" id="memEmail" value="<%=loginMem.getMemEmail() %>" required></td>
    ...
    function goBack(){	
		var stayOnPage = confirm("입력사항은 저장되지 않습니다.");
