...
public class OrderDetailDao {
	...
	public OrderDetailDao() {
		try {
			prop.loadFromXML(new FileInputStream(OrderDetailDao.class.getResource("/db/sql/orderr-mapper.xml").getPath()));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public ArrayList<OrderDetail> selectOrderDetail(Connection conn, int ono) {
		...
		//System.out.println("오더디테일다오odList : "+odList);
		return odList;
	}

	public int chStatus(Connection conn, int detailOno, String detailOnoText) {
		int result = 0;
		PreparedStatement pstmt = null;
		...
		try {
			...
			pstmt.setString(1, detailOnoText);
